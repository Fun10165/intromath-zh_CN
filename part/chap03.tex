...
\begin{example}
令 $A = \{1, 2, 2\}$, $B = \{1, 2, 3\}$, $C = \{3, 3, 1, 2\}$, $D = \{n \in \mathbb{N} : 1 \leq n \leq 3\}$, $E = \{2, 4\}$。
...
最后我们推广有关于有限集合差集的德摩根定律\index{德摩根定律（DeMorgan's laws）}（命题 3.2.9 (2)(3)）。
