...
给定一个关系 $S$ 之后，我们一般会用 $x \sim y$ 来表示 $(x, y)$ 是 $S$ 里的元素。这个符号 $x \sim y$ 的好处是容易让人感受 $x$ 和 $y$ 是有关系的，比较贴切“关系”字面上的意思。不过当我们要回归到利用集合的性质处理关系的问题时，回归到 $(x, y) \in S$ 的定义，会比较方便。
...
\begin{question}
对于任意正整数 $m$，令 $I = \{0, 1, \dots, m - 1\}$ 为指标集。考虑 $\mathbb{Z}$ 的划分，$C_i = \{mk + i : k \in \mathbb{Z}\}, i \in I$。试问此划分所对应的等价关系为何？
\end{question}
...
\begin{question}
令 $n$ 为正整数，$A = \{1, 2, \dots, n\}$ 且令 $X = \mathcal{P}(A)$。考虑 $X$ 上的关系，其定义为对任意 $B, C \in X, B \sim C$ 当且仅当 $\#(B) = \#(C)$。若 $m \in \mathbb{N}$ 且 $0 < m < n$，试问 $\{1, 2, \dots, m\}$ 所在的等价类其元素个数为何？试证明
...
$\mu = $ 极大元 & 是 & $(\lambda \in T) \wedge (\mu \preceq \lambda) \Rightarrow \lambda = \mu$ & $\mu$ 存在且为全序则 $\mu = g$ \\
$u = $ 上界 & 否 & $\forall t \in T, t \preceq u$ & $g$ 存在则 $g = \sup(T)$ \\
$\ell = $ 最小元 & 是 & $\forall t \in T, \ell \preceq t$ & $\ell$ 存在则 $\ell = m$ \\
$m = $ 极小元 & 是 & $(\lambda \in T) \wedge (\lambda \preceq m) \Rightarrow \lambda = m$ & $m$ 存在且为全序则 $m = \ell$ \\
